...
import {
  ...
  FormControl, InputLabel, Checkbox, FormControlLabel, Autocomplete, Chip
} from '@mui/material';
...
const MedecinDashboard = () => {
  const [data, setData] = useState({ medecin: null, institutions: [], availabilities: [], absences: [] });
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [medecinResponse, institutionsResponse, availabilitiesResponse, absencesResponse] = await Promise.all([
  ...
  const handleOpenAvailability = (availability = null) => {
    if (availability) {
      setAvailabilityForm({
        ...
        jours_semaine: [availability.jour_semaine], // Single day for editing
      ...
      <Dialog open={openAvailability} onClose={handleCloseAvailability} maxWidth="sm" fullWidth>
        ...
        <DialogContent>
          ...
          <Autocomplete
            ...
            disabled={availabilityForm.id !== null} // Disable institution change when editing
          />
          <FormControl fullWidth margin="dense">
            ...
            <Select
              ...
              disabled={availabilityForm.id !== null} // Disable day change when editing
            >
              {!availabilityForm.id && (
                <MenuItem value="all">
